...
module.exports = function(pool, minioClient, bucket) {
    const router = require('../lib/router').createRouter();
        const query = (text, values) => pool.query({ text, values, query_timeout: QUERY_TIMEOUT_MS });
    ...
    router.get('/library', exportLimiter, async (req, res) => {
        ...
        try {
            ...
            console.error('Export authorization error:', error);
        ...
        const state = { aborted: false, activeStream: null };
        const abort = () => {
